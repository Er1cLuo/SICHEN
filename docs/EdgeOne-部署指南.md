...
「公司位置」区块用高德**静态地图**显示公司位置，Key 通过环境变量注入，**不写进代码、不进 Git 仓库**。

| 变量名 | 值 | 必填 |
|---|---|---|
| `PUBLIC_AMAP_KEY` | 高德 Web 服务 Key（[高德开放平台](https://lbs.amap.com/) → 应用管理 → 创建应用 → 添加 Key，服务平台选 **Web服务**） | 是（不填则回退显示位置示意图，不影响其他功能） |
| `PUBLIC_AMAP_LOCATION` | 公司坐标，形如 `113.414460,22.348405`（GCJ-02） | 否（代码里已有默认值） |

配置位置：**EdgeOne Pages → 项目 → 项目设置 → 环境变量** → 新增上述变量 → **重新部署一次**（环境变量变更只对新部署生效）。

本地开发：复制 `.env.example` 为 `.env` 并填入同样的值（`.env` 已被 gitignore）。

> 注意：静态站点里的静态地图 URL 会包含 Key，前端可见——这是静态托管的固有限制。若担心被他人盗用配额，可在高德控制台给 Key 设置用量告警，或改用「JS API Key + 域名白名单」方案（可限制仅 `sicenhardware.com` 使用，需要改动代码，告诉我即可）。
